...
{
    /// <summary>
    /// MicroAPI核心逻辑，通过pathinfo值匹配不同的操作来处理http请求
    /// </summary>
    public abstract class MicroAPI : IHttpHandler
    {
        /// <summary>
    ...
    /// <summary>
    /// 泛型版的MicroAPI主要是针对处理请求是需要某些前置参数，比如仅限授权用户使用，TState就是User实例，您需要实现GetState来告知如何获取User，并根据StateNullHandler操作决定当User为空时是否继续执行
    /// </summary>
    /// <typeparam name="TState">前置设置的状态类型</typeparam>
    public abstract class MicroAPI<TState> : MicroAPI
    {
        /// <summary>
        ...
        protected void RegAction(string name, Action<HttpContext, TState> action)
        {
            Action<HttpContext> a = context =>
            {
        ...
        /// <param name="name"></param>
        /// <param name="action"></param>
        protected void RegTextAction(string name, Func<HttpContext, TState, string> action)
        {
        ...
        /// <param name="name"></param>
        /// <param name="action"></param>
        protected void RegJsonAction<T>(string name, Func<HttpContext, TState, T> action)
        {
            Action<HttpContext, TState> a = (context, state) =>
            {
        ...
        /// <param name="name"></param>
        /// <param name="action"></param>
        protected void RegXmlAction<T>(string name, Func<HttpContext, TState, T> action)
        {
            Action<HttpContext, TState> a = (context, state) =>
            {
                var result = action(context, state);
